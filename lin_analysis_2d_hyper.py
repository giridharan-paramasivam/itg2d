...
def init_linmats(pars,kx,ky):    
    ...
    lm[:,:,0,0]=-1j*sigk*D*kpsq**3-1j*sigk*H/kpsq**2
    lm[:,:,0,1]=(kapn+kapt)*ky
    lm[:,:,1,0]=-kapb*ky/Lk
    lm[:,:,1,1]=(kapn*ky-(kapn+kapt)*ky*kpsq)/Lk-1j*sigk*D*kpsq**3-1j*sigk*H/kpsq**2
...
Npx,Npy=4096,4096 # equivalent to Npx, Npy = 512, 512 in simulation
Nx,Ny=2*int(Npx/3),2*int(Npy/3)
# Lx,Ly=32*np.pi,32*np.pi # in simulation
...
D=1e-5 #0.1
H=1e-06 # 4.2581e-06 for kapt=0.5
base_pars={'kapn':kapn,
      'kapt':kapt,
      'kapb':kapb,
      'tau':1.,#Ti/Te
...
plt.plot(ky[0,:int(Ny/8)],-D*ky[0,:int(Ny/8)]**6,'k--',label='$-Dk_y^6$')
plt.legend()
plt.xlabel('$k_y$')
...
plt.axhline(0,color='k', linestyle='-', linewidth=1)
plt.plot(ky[0,:int(Ny/4)],-D*ky[0,:int(Ny/4)]**6,'k--')
plt.legend(['$k_x='+str(l)+'$' for l in kx[slx,0]]+['$-Dk_y^6$'])
plt.xlabel('$k_y$')
plt.ylabel('$\\gamma(k_y)$')
plt.tight_layout()
...
# plt.ylabel('$k_{y,max}$')
# plt.tight_layout()
# plt.savefig(f'data_linear/ky_vs_kx_kapt_{str(kapt).replace(".", "_")}_itg2d.svg', bbox_inches='tight')
# plt.show()
...
plt.axhline(0,color='k', linestyle='-', linewidth=1)
plt.plot(ky[0,1:int(Ny/8)],-D*ky[0,1:int(Ny/8)]**6,'k--',label='$-Dk_y^6$')
plt.legend()
plt.xlabel('$k_y$')
plt.ylabel('$\\left(\\frac{\\gamma}{k_y^2}\\right)$')
plt.tight_layout()
# plt.savefig(f'data_linear/Dturb_vs_ky_kapt_{str(kapt).replace(".", "_")}_itg2d.svg', bbox_inches='tight')
plt.show()

#%% Plot: colormesh of gam
...
plt.colorbar()
plt.tight_layout()
# plt.savefig(f'data_linear/gamkxky_kapt_{str(kapt).replace(".", "_")}_itg2d.svg', bbox_inches='tight')
plt.show()

#%% Plot: colormesh of omr
...
# plt.colorbar()
# plt.tight_layout()
# plt.savefig(f'data_linear/omrkxky_kapt_{str(kapt).replace(".", "_")}_itg2d.svg', bbox_inches='tight')
# plt.show()
